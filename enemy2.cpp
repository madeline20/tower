...
const QSize Enemy2::ms_fixedSize(100, 100);

Enemy2::Enemy2(WayPoint *startWayPoint, g2 *game, const QPixmap &sprite/* = QPixmap(":/image/2.png")*/,qreal speed,int Hp)
...
void Enemy2::move()
{
    ...
    // 向量标准化
    qreal movementSpeed = m_walkingSpeed*(1-m_slowedrate);
...
void Enemy2::draw(QPainter *painter) const
{
    ...
    QPoint healthBarPoint = m_pos + QPoint(-Health_Bar_Width / 2 - 5, -ms_fixedSize.height() / 3-30);
    // 绘制血条
...
bool Enemy2::getenterBase(){
    return m_enterBase;
}
void Enemy2::getSlowed(qreal slowingrate){
    m_slowedrate=slowingrate;
}
